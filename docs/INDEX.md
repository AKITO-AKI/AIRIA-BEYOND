...
このリポジトリのドキュメントは、散らばりを減らすために以下の **少数ファイルに統合**しました。
...
- 変更を本番に反映する手順（迷ったらここ）: [docs/DEPLOY_PLAYBOOK.md](DEPLOY_PLAYBOOK.md)
...
- 印刷用チラシ（QR/URL差し替え）: [docs/flyer.html](flyer.html)
- 旧ドキュメント（退避）: [docs/archive/](archive/)

> メモ: 旧ファイル名で参照しているリンクを壊さないため、代表的な旧ドキュメントは「スタブ（案内のみ）」として残しています。
